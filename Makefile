# Makefile generated by MakefileGenerator.cs
# *DO NOT EDIT*

UNREALROOTPATH = /mnt/primary-storage/Productivity/Development/Unreal/Engine/Linux_Unreal_Engine_5.4.3
GAMEPROJECTFILE =/mnt/primary-storage/Productivity/Development/Unreal/Projects/GMTK2024/GMTK2024.uproject

TARGETS = \
	UnrealEditor-Linux-DebugGame  \
	UnrealEditor-Linux-Shipping  \
	UnrealEditor \
	UnrealGame-Linux-DebugGame  \
	UnrealGame-Linux-Shipping  \
	UnrealGame \
	GMTK2024-Linux-DebugGame  \
	GMTK2024-Linux-Shipping  \
	GMTK2024 \
	GMTK2024Editor-Linux-DebugGame  \
	GMTK2024Editor-Linux-Shipping  \
	GMTK2024Editor\
	configure

BUILD = bash "$(UNREALROOTPATH)/Engine/Build/BatchFiles/Linux/Build.sh"
PROJECTBUILD = "$(UNREALROOTPATH)/Engine/Binaries/ThirdParty/DotNet/6.0.302/linux/dotnet" "$(UNREALROOTPATH)/Engine/Binaries/DotNET/UnrealBuildTool/UnrealBuildTool.dll"

all: StandardSet

RequiredTools: CrashReportClient-Linux-Shipping CrashReportClientEditor-Linux-Shipping ShaderCompileWorker UnrealLightmass EpicWebHelper-Linux-Shipping

StandardSet: RequiredTools UnrealFrontend GMTK2024Editor UnrealInsights

DebugSet: RequiredTools UnrealFrontend-Linux-Debug GMTK2024Editor-Linux-Debug


UnrealEditor-Linux-DebugGame:
	 $(BUILD) UnrealEditor Linux DebugGame  $(ARGS)

UnrealEditor-Linux-Shipping:
	 $(BUILD) UnrealEditor Linux Shipping  $(ARGS)

UnrealEditor:
	 $(BUILD) UnrealEditor Linux Development  $(ARGS)

UnrealGame-Linux-DebugGame:
	 $(BUILD) UnrealGame Linux DebugGame  $(ARGS)

UnrealGame-Linux-Shipping:
	 $(BUILD) UnrealGame Linux Shipping  $(ARGS)

UnrealGame:
	 $(BUILD) UnrealGame Linux Development  $(ARGS)

GMTK2024-Linux-DebugGame:
	 $(PROJECTBUILD) GMTK2024 Linux DebugGame  -project="$(GAMEPROJECTFILE)" $(ARGS)

GMTK2024-Linux-Shipping:
	 $(PROJECTBUILD) GMTK2024 Linux Shipping  -project="$(GAMEPROJECTFILE)" $(ARGS)

GMTK2024:
	 $(PROJECTBUILD) GMTK2024 Linux Development  -project="$(GAMEPROJECTFILE)" $(ARGS)

GMTK2024Editor-Linux-DebugGame:
	 $(PROJECTBUILD) GMTK2024Editor Linux DebugGame  -project="$(GAMEPROJECTFILE)" $(ARGS)

GMTK2024Editor-Linux-Shipping:
	 $(PROJECTBUILD) GMTK2024Editor Linux Shipping  -project="$(GAMEPROJECTFILE)" $(ARGS)

GMTK2024Editor:
	 $(PROJECTBUILD) GMTK2024Editor Linux Development  -project="$(GAMEPROJECTFILE)" $(ARGS)

configure:
	xbuild /property:Configuration=Development /verbosity:quiet /nologo "$(UNREALROOTPATH)/Engine/Source/Programs/UnrealBuildTool/UnrealBuildTool.csproj"
	$(PROJECTBUILD) -projectfiles -project="\"$(GAMEPROJECTFILE)\"" -game -engine 

.PHONY: $(TARGETS)
